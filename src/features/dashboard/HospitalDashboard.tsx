import { useState } from 'react';
import { Building2, Package, AlertTriangle, TrendingUp, Clock, Droplets } from 'lucide-react';
import { AreaChart, Area, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer } from 'recharts';
import { cn, formatBloodGroup, COMPONENT_LABELS } from '@/lib/utils';
import { DEMO_ORGANIZATIONS, DEMO_INVENTORY, generateDemoForecasts, DEMO_REQUESTS } from '@/lib/demo-data';

const CustomTooltip = ({ active, payload, label }: any) => {
  if (active && payload && payload.length) {
    const actual = payload.find((p: any) => p.dataKey === 'actual')?.value;
    const forecast = payload.find((p: any) => p.dataKey === 'forecast')?.value;
    const variance = (actual !== undefined && forecast !== undefined) ? actual - forecast : null;

    return (
      <div className="bg-[#FFFFFF] border border-[#E2E2DC] rounded-lg p-3 shadow-soft text-xs space-y-1">
        <p className="font-bold text-[#1A1F26] border-b border-[#E2E2DC] pb-1 font-mono">{label}</p>
        <div className="flex items-center gap-2 pt-1">
          <span className="w-2.5 h-2.5 rounded-full bg-[#5B8C7A]" />
          <span className="text-[#64748B]">Actual Demand:</span>
          <span className="font-bold text-[#1A1F26] ml-auto">{actual} Units</span>
        </div>
        <div className="flex items-center gap-2">
          <span className="w-2.5 h-2.5 rounded-full bg-[#C85A3F]" />
          <span className="text-[#64748B]">AI Forecast:</span>
          <span className="font-bold text-[#1A1F26] ml-auto">{forecast} Units</span>
        </div>
        {variance !== null && (
          <div className="flex items-center gap-2 pt-1 border-t border-[#E2E2DC]/50 mt-1 text-[11px]">
            <span className="text-[#64748B]">Variance:</span>
            <span className={cn('font-bold ml-auto font-mono', variance >= 0 ? 'text-[#5B8C7A]' : 'text-[#C85A3F]')}>
              {variance >= 0 ? `+${variance}` : variance} Units
            </span>
          </div>
        )}
      </div>
    );
  }
  return null;
};

export function HospitalDashboard() {
  const [selectedHospId, setSelectedHospId] = useState('HOSP_A');
  const hospitals = DEMO_ORGANIZATIONS.filter(o => o.type === 'HOSPITAL');
  const currentHosp = hospitals.find(h => h.id === selectedHospId) || hospitals[0];

  const inventory = DEMO_INVENTORY.filter(i => i.organizationId === selectedHospId);
  const requests = DEMO_REQUESTS.filter(r => r.organizationId === selectedHospId);

  const forecastData = [
    { date: '01 Sep', actual: 42, forecast: 40 },
    { date: '02 Sep', actual: 48, forecast: 45 },
    { date: '03 Sep', actual: 39, forecast: 41 },
    { date: '04 Sep', actual: 52, forecast: 49 },
    { date: '05 Sep', actual: 46, forecast: 44 },
    { date: '06 Sep (Est)', actual: undefined, forecast: 48 },
    { date: '07 Sep (Est)', actual: undefined, forecast: 50 },
  ];

  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      {/* Selector & Header */}
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-2 border-b border-[#E2E2DC]">
        <div>
          <h2 className="text-xl font-bold text-[#1A1F26] tracking-tight">{currentHosp.name}</h2>
          <p className="text-xs text-[#64748B] mt-0.5">
            Hospital node dashboard • {currentHosp.bedCapacity} beds • Regional Node: {currentHosp.region}
          </p>
        </div>

        <div className="flex items-center gap-2">
          <span className="text-xs text-[#64748B] font-medium">Select Hospital:</span>
          <select
            value={selectedHospId}
            onChange={e => setSelectedHospId(e.target.value)}
            className="px-3 py-1.5 rounded-lg border border-[#E2E2DC] bg-[#FFFFFF] text-xs font-semibold text-[#1A1F26] focus:outline-none"
          >
            {hospitals.map(h => (
              <option key={h.id} value={h.id}>{h.name}</option>
            ))}
          </select>
        </div>
      </div>

      {/* KPI Cards */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-[#FFFFFF] border border-[#E2E2DC] rounded-xl p-4 shadow-flat">
          <p className="text-2xl font-bold text-[#5B8C7A] font-mono">
            {inventory.reduce((sum, item) => sum + item.availableUnits, 0)}
          </p>
          <p className="text-xs text-[#64748B] mt-0.5">Available Stock</p>
        </div>
        <div className="bg-[#FFFFFF] border border-[#E2E2DC] rounded-xl p-4 shadow-flat">
          <p className="text-2xl font-bold text-[#5C768D] font-mono">
            {inventory.reduce((sum, item) => sum + item.reservedUnits, 0)}
          </p>
          <p className="text-xs text-[#64748B] mt-0.5">Reserved Units</p>
        </div>
        <div className="bg-[#FFFFFF] border border-[#E2E2DC] rounded-xl p-4 shadow-flat">
          <p className="text-2xl font-bold text-[#D99B38] font-mono">
            {inventory.reduce((sum, item) => sum + item.nearExpiryUnits, 0)}
          </p>
          <p className="text-xs text-[#64748B] mt-0.5">Near Expiry (&lt;72h)</p>
        </div>
        <div className="bg-[#FFFFFF] border border-[#E2E2DC] rounded-xl p-4 shadow-flat">
          <p className="text-2xl font-bold text-[#C85A3F] font-mono">{requests.length}</p>
          <p className="text-xs text-[#64748B] mt-0.5">Active Requests</p>
        </div>
      </div>

      {/* Inventory Grid & Forecast Chart */}
      <div className="grid lg:grid-cols-3 gap-6">
        {/* Full Blood Inventory Matrix (2 cols) */}
        <div className="lg:col-span-2 bg-[#FFFFFF] border border-[#E2E2DC] rounded-xl p-5 shadow-flat">
          <div className="flex items-center justify-between pb-3 border-b border-[#E2E2DC] mb-3">
            <h3 className="text-sm font-bold text-[#1A1F26]">Blood Group Inventory Matrix</h3>
            <span className="text-xs text-[#64748B]">Real-time unit levels</span>
          </div>

          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead>
                <tr className="text-left text-[#64748B] border-b border-[#E2E2DC]">
                  <th className="pb-2 font-medium">Group</th>
                  <th className="pb-2 font-medium">Component</th>
                  <th className="pb-2 font-medium text-right">Available</th>
                  <th className="pb-2 font-medium text-right">Reserved</th>
                  <th className="pb-2 font-medium text-right">Near Expiry</th>
                  <th className="pb-2 font-medium text-right">Safety Target</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[#E2E2DC]">
                {inventory.map((inv, idx) => (
                  <tr key={idx} className="hover:bg-[#F7F7F5] transition-colors">
                    <td className="py-2.5 font-bold text-[#C85A3F]">{formatBloodGroup(inv.bloodGroup)}</td>
                    <td className="py-2.5 text-[#1A1F26]">{COMPONENT_LABELS[inv.componentType]}</td>
                    <td className="py-2.5 text-right font-mono font-bold text-[#5B8C7A]">{inv.availableUnits}</td>
                    <td className="py-2.5 text-right font-mono text-[#5C768D]">{inv.reservedUnits}</td>
                    <td className="py-2.5 text-right font-mono text-[#D99B38]">{inv.nearExpiryUnits}</td>
                    <td className="py-2.5 text-right font-mono text-[#64748B]">{inv.safetyStockTarget}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
        </div>

        {/* Hospital Demand Chart (1 col) */}
        <div className="bg-[#FFFFFF] border border-[#E2E2DC] rounded-xl p-5 shadow-flat flex flex-col justify-between">
          <div className="pb-2 border-b border-[#E2E2DC] mb-2">
            <h3 className="text-sm font-bold text-[#1A1F26]">Hospital Demand Trend</h3>
            <p className="text-[11px] text-[#64748B]">Daily usage vs AI projection</p>
          </div>

          <div className="h-56 w-full pt-2">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={forecastData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#E2E2DC" vertical={false} />
                <XAxis dataKey="date" tick={{ fontSize: 9, fill: '#64748B' }} stroke="#E2E2DC" />
                <YAxis tick={{ fontSize: 9, fill: '#64748B' }} stroke="#E2E2DC" />
                <Tooltip content={<CustomTooltip />} />
                <Area type="monotone" dataKey="actual" stroke="#5B8C7A" strokeWidth={2} fill="#5B8C7A" fillOpacity={0.1} />
                <Area type="monotone" dataKey="forecast" stroke="#C85A3F" strokeWidth={2} strokeDasharray="4 4" fill="none" />
              </AreaChart>
            </ResponsiveContainer>
          </div>

          <div className="pt-2 border-t border-[#E2E2DC] text-[11px] text-[#64748B]">
            <p>Target Safety Stock: <strong className="text-[#1A1F26]">120 Units</strong></p>
          </div>
        </div>
      </div>
    </div>
  );
}
